...
/* ----------------------------------------------------------------------------
|                               Added by Kamyar                               |
|                   To get nice name for each security type                   |
---------------------------------------------------------------------------- */
function stringifyNMAccessPointSecurity(val) {
    if (val === NMAccessPointSecurity.UNKNOWN)
        return 'Unknown';
    if (val === NMAccessPointSecurity.NONE)
        return 'None';
    if (val === NMAccessPointSecurity.WEP)
        return 'WEP';
    if (val === NMAccessPointSecurity.WPA_PSK)
        return 'WPA PSK';
    if (val === NMAccessPointSecurity.WPA2_PSK)
        return 'WPA2 PSK';
    if (val === NMAccessPointSecurity.WPA_ENT)
        return 'WPA ENT';
    if (val === NMAccessPointSecurity.WPA2_ENT)
        return 'WPA2 ENT';
    return 'NULL';
}
/* ----------------------------------------------------------------------------
